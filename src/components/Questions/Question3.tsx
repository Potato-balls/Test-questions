    ...
    <div className="q3-page">
      <div className="letter-container">
        ...
        <div className="letter-header">
          ...
          <div className="letter-date">2026年9月16日</div>
        </div>

        {/* 信纸正文 */}
        <div className="letter-body">
          <div className="letter-greeting">您好，</div>
          <div className="letter-content">
            {LETTER_CONTENT.split('\n').map((paragraph, idx) => (
              paragraph.trim() ? (
                <p key={idx} className={`letter-paragraph ${paragraph.startsWith('一、') || paragraph.startsWith('二、') || paragraph.startsWith('三、') ? 'letter-section-title' : ''}`}>
                  {paragraph}
                </p>
              ) : null
            ))}
        ...
        <div className="letter-footer">
          <div className="letter-watermark">面试材料</div>
        </div>
      </div>
    </div>
  );
